        ...
        if message.Command == "rustle":
            if len(message.ParameterList) != 1:
                ...
                if roll == 1:
                    ...
                    return IRCResponse(ResponseType.Say, message.ParameterList[0] + "'s jimmies status: unrustled", message.ReplyTo)
                elif (roll > 11) and (roll < 20):
                    return IRCResponse(ResponseType.Say, message.ParameterList[0] + "'s jimmies status: rustled", message.ReplyTo)
                else:
                    return IRCResponse(ResponseType.Say, message.ParameterList[0] + "'s jimmies status: CRITICAL RUSTLE", message.ReplyTo)
